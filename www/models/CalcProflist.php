...
class CalcProflist  extends FuncZabor {
	...
	public $proflist_nado;			//только профлист нужен без металлического каркаса
	...
	public function attributeLabels()
    {
        return [
			...
			'proflist_type'=>'Тип профлиста',
			'proflist_color'=>'Цвет профлиста',
			'proflist_count'=>'Листов',
			'planka'=>'Тип планки',
	...
	public function scenarios()
  {
      $scenarios = parent::scenarios();
		$scenarios['step1'] = ['stolb_nado', 'laga_nado', 'proflist_nado','rabota_nado', 'focus_input','len', 'height','stolb_razmer', 'stolb_len','stolb_height','stolb_glubina','stolb_count','stolb_krishka',
		...
		'proflist_type', 'proflist_color', 'proflist_count','planka','planka_count', 'samorez_count','samorez_color','samorez_type'];
        return $scenarios;
  }
    ...
    public function rules()
    {
		...
		[
			[['focus_input'],'string'],
			[['stolb_count','stolb_otkos','laga_kol','laga_vsego','proflist_count','planka_count', 'samorez_count'],'integer'],
			...
			'proflist_type','proflist_color' ,'planka','samorez_color','samorez_type'], 'required','on'=>'step1']
        ];
    }
	...
	public function calc_stolb() {
		...
		$tmp=0;
		/*
		if ($this->focus_input=='calcproflist-height') {
		...
		*/
		 ...
		 if(($this->focus_input=='calcproflist-stolb_glubina') || ($this->focus_input=='calcproflist-height') ){
		 ...
		 } elseif (( $this->focus_input=='calcproflist-stolb_butov') ) {
		 ...
		 } elseif (($this->focus_input=='calcproflist-stolb_beton') ){
			 $this->stolb_glubina='zaglub_none';
			 $this->stolb_butov='butov_none';
	...
	public function calcInit1() { // устанавливаем значения при первом открытии формы
	    ...
	    $this->proflist_nado=1;			//только профлист нужен без металлического каркаса
		...
		$this->proflist_type='op_c_8_0_4';			
		$this->proflist_color='color_none';			
		$this->planka='planka_standart';						
		$this->samorez_color='kraska_none';				
		$this->proflist_type_text=$this->readElements(5,0)+$this->readElements(6,0)+$this->readElements(7,0)+$this->readElements(8,0);		//типы профлиста
		//в зависимости от типа профлиста выбираем возможные варианты цвета
		//читаем первые две буквы типа профлиста
	
		if (substr($this->proflist_type,0,2)=='ci') { // первые две буквы атрибута пример cink_c_8_0_4
			$this->proflist_color_text=$this->readElements(10,4);								// цвет профлиста
		};
		if (substr($this->proflist_type,0,2)=='op') {
			$this->proflist_color_text=$this->readElements(10,1)+$this->readElements(10,2);								// цвет профлиста
		};
		if (substr($this->proflist_type,0,2)=='dp') {
			$this->proflist_color_text=$this->readElements(10,2);								// цвет профлиста
		};		
		if (substr($this->proflist_type,0,2)=='eo') {
			$this->proflist_color_text=$this->readElements(10,3);								// цвет профлиста
		};	
		$this->proflist_count=0;																	// общее количество профлистов
		$this->planka=0;
		$this->planka_text=$this->readElements(11,0);								//текста для декоративной планки
		$this->planka_count=0;
		$this->samorez_count=0;																//количество саморезов всего
		$this->samorez_color=0;																	//цвет самореза
		...
		// рассчитываем количество лаг по высоте
		 $this->laga_kol=2;
		 if ($this->height >2) 	{ $this->laga_kol=3;	 }
		 if ($this->height >2.5) { $this->laga_kol=4;	 }
		return;
	}
	...
	public function calcStep1() { 	// расчет после изменения любого из значений формы
		...
		$this->proflist_count=ceil ($this->len / $this->getelement($this->proflist_type));	// длину делим на ширину профлиста и округляем в большую сторону
		$this->planka_text=$this->readElements(11,0);								//текста для декоративной планки
		...
		$this->samorez_count=$this->laga_vsego*15;								//количество саморезов всего
		$this->samorez_color_text=$this->readElements(10,1)+$this->readElements(10,4);					//цвета саморезов
	...
	public function calc ($c,$s,$smeta) {
			...
			//***************************  СОХРАНЯЕМ СМЕТУ НА ПРОФЛИСТ  ***********************
				...
				// ########  сохраняем тип : профлист, рабица, штакет...
					...
					$smeta=new Smeta(); $smeta->user_id=$c; 	$smeta->smeta_id=$s;  $smeta->datetime=date("Y-m-d H:i:s");  $smeta->zabortype='proflist';
					$smeta->attr   = 'proflist'; 
					$smeta->type  = $s; 
					$smeta->kol  = 0;
					$smeta->ed='';
