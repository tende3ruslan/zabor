...
$this->title = 'РАСЧЕТ РАСПАШНЫХ ВОРОТ';
...
<?php $form = ActiveForm::begin(['action' =>['site/calcvorota'], 'options' => ['data-pjax' => true ], 'method' => 'post',  'id' => 'form-signup'] ); ?>
...
<div class="col-xs-2">
<?=
$form->field($model, 'stopor')
    ->dropDownList($model->stopor_text,
    [ 'options' => [$model->stopor=>['selected' => 'selected']]]
...
<div class="col-xs-2">
<?=
$form->field($model, 'otkrivanie')
    ->dropDownList($model->otkrivanie_text,
    [ 'options' => [$model->otkrivanie=>['selected' => 'selected']]]
...
$(document).on('click', '#btn-smeta', function() {
	  ...
	  if ($("#calcvorota-len").val() == "") {
		alert('Укажите длину забора');
		return;
    }
	
	if ($("#calcvorota-height").val() == "") {
		alert('Укажите высоту забора');
		return;
    }
	
    
	 $("#calcvorota-focus_input").val('smeta');
	 
	  
	 //Необходимо прокрутить в конец страницы
	var scrollTime=500;
	$("body,html").animate({"scrollTop":380},scrollTime);
...
$(document).on('click', '#calc_clear', function() {
	 $("#calcvorota-focus_input").val('clear');
	  $("#form-signup").submit();
});
