...
$this->title = 'РАСЧЕТ ЛЕНТОЧНОГО ФУНДАМЕНТА';
...
<?php $form = ActiveForm::begin(['action' =>['site/calcfundament'], 'options' => ['data-pjax' => true ], 'method' => 'post',  'id' => 'form-signup'] ); ?>
...
$form->field($model, 'width')
    ->dropDownList($model->width_text,
    [ 'options' => [$model->width=>['selected' => 'selected']]]
...
<div class="col-xs-2">
<?= $form->field($model,"zaglublenie"); ?>
</div>

<div class="col-xs-2">
<?= $form->field($model,"nad")->textInput(['readonly' => 'true']); ?>
</div>
...
<div class="col-xs-2">
<?= $form->field($model,"len_stolb"); ?>
</div>

<div class="col-xs-2">
<?=
$form->field($model, 'armokarkas')
    ->dropDownList($model->armokarkas_text,
    [ 'options' => [$model->armokarkas=>['selected' => 'selected']]]
...
$(document).on('click', '#btn-smeta', function() {
	  ...
	  if ($("#calcfundament-len1").val() == "") {
		alert('Укажите длину забора');
		return;
    }
	
	if ($("#calcfundament-height1").val() == "") {
		alert('Укажите высоту забора');
		return;
    }
	
    
	 $("#calcfundament-focus_input").val('smeta');
...
$(document).on('click', '#calc_clear', function() {
	 $("#calcfundament-focus_input").val('clear');
	  $("#form-signup").submit();
});
