...
$this->title = 'РАСЧЕТ ОТКАТНЫХ ВОРОТ';
...
<?php $form = ActiveForm::begin(['action' =>['site/calcotkatnievorota'], 'options' => ['data-pjax' => true ], 'method' => 'post',  'id' => 'form-signup'] ); ?>
...
<div class="col-xs-2">
<?=
$form->field($model, 'avtomatika')
    ->dropDownList($model->avtomatika_text,
    [ 'options' => [$model->avtomatika=>['selected' => 'selected']]]
...
<div class="col-xs-3">
<?=
$form->field($model, 'tumba')
    ->dropDownList($model->tumba_text,
    [ 'options' => [$model->tumba=>['selected' => 'selected']]]
...
$(document).on('click', '#btn-smeta', function() {
	  ...
	  if ($("#calcotkatnievorota-len1").val() == "") {
		alert('Укажите длину забора');
		return;
    }
	
	if ($("#calcotkatnievorota-height1").val() == "") {
		alert('Укажите высоту забора');
		return;
    }
	
    
	 $("#calcotkatnievorota-focus_input").val('smeta');
...
$(document).on('click', '#calc_clear', function() {
	 $("#calcotkatnievorota-focus_input").val('clear');
	  $("#form-signup").submit();
});
