...
<?php $form = ActiveForm::begin(['action' =>['site/calckalitka'], 'options' => ['data-pjax' => true ], 'method' => 'post',  'id' => 'form-signup'] ); ?>
...
<div class="col-xs-3">
<?=
$form->field($model, 'nastil')
    ->dropDownList($model->nastil_text,
    [ 'options' => [$model->nastil=>['selected' => 'selected']]]	  
	);
	?>
</div>
...
<div class="col-xs-2">
<?=
$form->field($model, 'stolb_count')
    ->dropDownList($model->stolb_count_text,
    [ 'options' => [$model->stolb_count=>['selected' => 'selected']]]
...
<div class="col-xs-3">
<?=
$form->field($model, 'open_type')
    ->dropDownList($model->open_type_text,
    [ 'options' => [$model->open_type=>['selected' => 'selected']]]
...
$form->field($model, 'side')
    ->dropDownList($model->side_text,
    [ 'options' => [$model->side=>['selected' => 'selected']]]
...
<div class="col-xs-3">
<?=
$form->field($model, 'stajka')
    ->dropDownList($model->stajka_text,
    [ 'options' => [$model->stajka=>['selected' => 'selected']]]
...
<div class="col-xs-3">
<?=
$form->field($model, 'zamok')
    ->dropDownList($model->zamok_text,
    [ 'options' => [$model->zamok=>['selected' => 'selected']]]
...
<div class="col-xs-3">
<?=
$form->field($model, 'zadvigka')
    ->dropDownList($model->zadvigka_text,
    [ 'options' => [$model->zadvigka=>['selected' => 'selected']]]
...
<div class="col-xs-3">
<?=
$form->field($model, 'ruchka')
    ->dropDownList($model->ruchka_text,
    [ 'options' => [$model->ruchka=>['selected' => 'selected']]]
...
<div class="col-xs-3">
<?=
$form->field($model, 'ukosin_count')
    ->dropDownList($model->ukosin_count_text,
    [ 'options' => [$model->ukosin_count=>['selected' => 'selected']]]
...
<div class="col-xs-3">
<?=
$form->field($model, 'otkos_count')
    ->dropDownList($model->otkos_count_text,
    [ 'options' => [$model->otkos_count=>['selected' => 'selected']]]
...
<div class="col-xs-2">
<?= $form->field($model,"kalitka_otdelno")->checkbox(); ?>
</div>

<div class="row" ></div>
<div class="row" style="padding-left:30px;padding-bottom:10px; font-weight:700">
Установлены значения из предыдущих расчетов клиента (тип листа, грунтовка, покраска, заглубление)
</div>
<div class="col-xs-3">
<?=
$form->field($model, 'pokritie')
    ->dropDownList($model->pokritie_text,
    [ 'options' => [$model->pokritie=>['selected' => 'selected']]]
...
<div class="col-xs-3">
<?=
$form->field($model, 'stolb_zaglub')
    ->dropDownList($model->stolb_zaglub_text,
    [ 'options' => [$model->stolb_zaglub=>['selected' => 'selected']]]
...
<div id="smeta" style="height:500px">
<div class="row">
...
$(document).on('click', '#btn-smeta', function() {
	  ...
	  if ($("#calckalitka-len1").val() == "") {
		alert('Укажите длину забора');
		return;
    }
	
	if ($("#calckalitka-height1").val() == "") {
		alert('Укажите высоту забора');
		return;
    }
	
    
	 $("#calckalitka-focus_input").val('smeta');
	 
	  
	 //Необходимо прокрутить в конец страницы
	var scrollTime=500;
	$("body,html").animate({"scrollTop":500},scrollTime);
...
$(document).on('click', '#calc_clear', function() {
	 $("#calckalitka-focus_input").val('clear');
	  $("#form-signup").submit();
});
