...
$this->title = 'РАСЧЕТ ПАРКОВОЧНОГО МЕСТА';
...
<?php $form = ActiveForm::begin(['action' =>['site/calcparkovka'], 'options' => ['data-pjax' => true ], 'method' => 'post',  'id' => 'form-signup'] ); ?>
...
$(document).on('click', '#btn-smeta', function() {
	  ...
	  if ($("#calcparkovka-len1").val() == "") {
		alert('Укажите длину забора');
		return;
    }
	
	if ($("#calcparkovka-height1").val() == "") {
		alert('Укажите высоту забора');
		return;
    }
	
    
	 $("#calcparkovka-focus_input").val('smeta');
...
$(document).on('click', '#calc_clear', function() {
	 $("#calcparkovka-focus_input").val('clear');
	  $("#form-signup").submit();
});
