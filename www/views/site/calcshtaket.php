...
$this->title = 'РАСЧЕТ ЗАБОРА ИЗ ШТАКЕТА';
...
<?php $form = ActiveForm::begin(['action' =>['site/calcshtaket'], 'options' => ['data-pjax' => true ], 'method' => 'post',  'id' => 'form-signup'] ); ?>
...
<?= $form->field($model,"laga_nado")->checkbox(); ?>
</div>

<div class="col-xs-2">
<?= $form->field($model,"shtaket_nado")->checkbox(); ?>
...
<?=
$form->field($model, 'laga_razmer')
    ->dropDownList($model->laga_razmer_text,
    [ 'options' => [$model->laga_razmer=>['selected' => 'selected']]]
...
<div class="col-xs-2">
<?= $form->field($model,"laga_kol"); ?>
</div>


<div class="col-xs-2">
<?= $form->field($model,"laga_vsego")->textInput(['readonly' => 'true']); ?>
</div>
...
<?=
$form->field($model, 'shtaket_type')
    ->dropDownList($model->shtaket_type_text,
    [ 'options' => [$model->shtaket_type=>['selected' => 'selected']]]
...
<?=
$form->field($model, 'shtaket_verx')
    ->dropDownList($model->shtaket_verx_text,
    [ 'options' => [$model->shtaket_verx=>['selected' => 'selected']]]
...
<?=
$form->field($model, 'shtaket_radov')
    ->dropDownList($model->shtaket_radov_text,
    [ 'options' => [$model->shtaket_radov=>['selected' => 'selected']]]
...
<div class="col-xs-1">
<?= $form->field($model,"shtaket_zazor"); ?>
</div>


<div class="col-xs-1">
<?= $form->field($model,"shtaket_count")->textInput(['readonly' => 'true']); ?>
</div>
...
<?=
$form->field($model, 'shtaket_color')
    ->dropDownList($model->shtaket_color_text,
    [ 'options' => [$model->shtaket_color=>['selected' => 'selected']]]
...
<?=
$form->field($model, 'planka')
    ->dropDownList($model->planka_text,
    [ 'options' => [$model->planka=>['selected' => 'selected']]]
...
<div class="col-xs-2">
<?= $form->field($model,"planka_count"); ?>
</div>


<div class="col-xs-2">
<?= $form->field($model,"samorez_count")->textInput(['readonly' => 'true']); ?>
</div>
...
<?=
$form->field($model, 'samorez_color')
    ->dropDownList($model->samorez_color_text,
    [ 'options' => [$model->samorez_color=>['selected' => 'selected']]]
...
<?=
$form->field($model, 'samorez_type')
    ->dropDownList($model->samorez_type_text,
    [ 'options' => [$model->samorez_type=>['selected' => 'selected']]]
...
<?php if (($item['price'] > 0) && ($item['kol']>0)) {?>
	...
	<div class="col-xs-7"><?= ($item['table'][0]=='e') ?  $model->getelement_name($item['attr']) : $model->getwork_name($item['attr']) ?> <?= $item['type'] ?></div>
...
//если выбран первый или второй инпут формируем ссылку back с передачей h и w
 ...
 $("a[name='link-back']").attr('href', '/web/index.php?r=site%2Fcalc&h=' + $('#calcshtaket-height').val() +  '&l=' + $('#calcshtaket-len').val() );	
...
$(document).on('change', '.form-control:gt(1)', function() {
 ...
 $("#calcshtaket-focus_input").val(tmp);
...
$(document).on('click', '#btn-smeta', function() {
	  ...
	  if ($("#calcshtaket-len").val() == "") {
		alert('Укажите длину забора');
		return;
    }
	
	if ($("#calcshtaket-height").val() == "") {
		alert('Укажите высоту забора');
		return;
    }
	
    if (($("#calcshtaket-stolb_glubina").val() == "zaglub_none") && ($("#calcshtaket-stolb_butov").val() == "butov_none") && ($("#calcshtaket-stolb_beton").val() == "beton_none")) {
		alert('Укажите вариант заглубления, для расчета высоты столбов');
		return;
    }
    
	 $("#calcshtaket-focus_input").val('smeta');
...
$(document).on('click', '#calc_clear', function() {
	 $("#calcshtaket-focus_input").val('clear');
	  $("#form-signup").submit();
});
