...
<?php $form = ActiveForm::begin(['action' =>['site/calctransport'], 'options' => ['data-pjax' => true ], 'method' => 'post',  'id' => 'form-signup'] ); ?>
...
<div class="col-xs-2">
<?=
$form->field($model, 'ves')
    ->dropDownList($model->ves_text,
    [ 'options' => [$model->ves=>['selected' => 'selected']]]	  
	);
	?>
</div>

<div class="col-xs-2">
<?= $form->field($model,"reisov"); ?>
</div>
...
<?php if (($item['price'] > 0) && ($item['kol']>0)) {?>
	...
	<div class="col-xs-7"><?= $model->getelement_name($item['attr']).' '.$item['type'] ?></div>
...
$(document).on('change', '.form-control:gt(1)', function() {
 ...
 $("#calctransport-focus_input").val(tmp);
...
$(document).on('click', '#btn-smeta', function() {
	  ...
	  if ($("#calctransport-len").val() == "") {
		alert('Укажите расстояние');
		return;
    }
    
	 $("#calctransport-focus_input").val('smeta');
...
$(document).on('click', '#calc_clear', function() {
	 $("#calctransport-focus_input").val('clear');
	  $("#form-signup").submit();
});
