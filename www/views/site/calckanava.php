...
$this->title = 'РАСЧЕТ ВЬЕЗДА ЧЕРЕЗ КАНАВУ (Ширина - это максимальная ширина машины)';
...
<?php $form = ActiveForm::begin(['action' =>['site/calckanava'], 'options' => ['data-pjax' => true ], 'method' => 'post',  'id' => 'form-signup'] ); ?>
...
<div class="col-xs-3">
<?=
$form->field($model, 'betonirovanie')
    ->dropDownList($model->betonirovanie_text,
    [ 'options' => [$model->betonirovanie=>['selected' => 'selected']]]	  
	);
	?>
</div>
...
$(document).on('click', '#btn-smeta', function() {
	  ...
	  if ($("#calckanava-len1").val() == "") {
		alert('Укажите длину забора');
		return;
    }
	
	if ($("#calckanava-height1").val() == "") {
		alert('Укажите высоту забора');
		return;
    }
	
    
	 $("#calckanava-focus_input").val('smeta');
...
$(document).on('click', '#calc_clear', function() {
	 $("#calckanava-focus_input").val('clear');
	  $("#form-signup").submit();
});
